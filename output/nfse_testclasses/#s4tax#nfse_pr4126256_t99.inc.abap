*&---------------------------------------------------------------------*
*& Include #s4tax#nfse_pr4126256_t99.inc
*&---------------------------------------------------------------------*
CLASS ltcl_nfse_pr4126256 DEFINITION FINAL FOR TESTING
  INHERITING FROM /s4tax/nfse_default_test
    DURATION SHORT
    RISK LEVEL HARMLESS.

  PRIVATE SECTION.
    DATA: cut TYPE REF TO /s4tax/nfse_pr4126256.
...
CLASS ltcl_nfse_pr4126256 IMPLEMENTATION.
  ...
  METHOD get_rps_identificacao.
    ...
    me->tax_address_branch_adr->set_taxjurcode( 'PR4126256' ).
    DATA(result) = cut->get_rps_identificacao( ).
    cl_abap_unit_assert=>assert_equals( exp = 'NF' act = result-serie ).
    cl_abap_unit_assert=>assert_equals( exp = '1' act = result-natureza_operacao ).
  ENDMETHOD.
  ENDMETHOD.

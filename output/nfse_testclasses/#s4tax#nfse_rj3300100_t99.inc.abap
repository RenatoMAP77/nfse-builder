*&---------------------------------------------------------------------*
*& Include #s4tax#nfse_rj3300100_t99.inc
*&---------------------------------------------------------------------*
CLASS ltcl_nfse_rj3300100 DEFINITION FINAL FOR TESTING
  INHERITING FROM /s4tax/nfse_default_test
    DURATION SHORT
    RISK LEVEL HARMLESS.

  PRIVATE SECTION.
    DATA: cut TYPE REF TO /s4tax/nfse_rj3300100.
...
CLASS ltcl_nfse_rj3300100 IMPLEMENTATION.
  ...
  METHOD get_rps_identificacao.
    DATA(result) = cut->get_rps_identificacao( ).
    cl_abap_unit_assert=>assert_equals( exp = 'NF' act = result-serie ).
    cl_abap_unit_assert=>assert_not_initial( result-natureza_operacao ).
  ENDMETHOD.
  ...
  METHOD get_rps_tomador.
    DATA(result) = cut->get_rps_tomador( ).
    cl_abap_unit_assert=>assert_not_initial( result-cpf ).
  ENDMETHOD.
